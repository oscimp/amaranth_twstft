...
dirlist=dir([datalocation,'/*_',num2str(3-remotechannel),'.bin']);
  ...
  if ((exist(nomout)==0)&&(exist(nomoutgz)==0))
    ...
    if (remote!=1)
       ...
       if (OP==1)
           ...
           k=find((freq<120000)&(freq>100000));
       end
    end
    dold=[];
    moved=[];
...
%           d2=fftshift(abs(fft(d(remotechannel:2:end).^2))); % adjust for remote channel vs remote
%           d=[dold ; d(remotechannel:2:end)];                % adjust for remote channel vs remote
%        else                  % vvv 0.5 Hz accuracy
           d2=fftshift(abs(fft(d(1:end).^2))); % adjust for local channel vs remote
           d=[dold ; d(1:end)];                % adjust for local channel vs remote
%        end
