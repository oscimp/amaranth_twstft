...
In order to avoid these conditions, it may be necessary to delay the input PPS to ensure that it will always be detected during the same tick.
...
By default, the calibration sequence is launched at reset and when a bad PPS is detected.
To ask the phase number of the calibration (and do the calibration if not already done) :
```
