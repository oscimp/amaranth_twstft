...
if (longueur==4*fs)  
    ...
    k=find((freq<-50000)&(freq>-150000));
    d22=fftshift(abs(fft(d1.^2))); % 0.1 Hz accuracy
%if (p==1) plot(d22(k));end
...
printf("%f %f\n",indice1(p))
%figure
%plot(abs(prnmap01(indice1(p)-1000:indice1(p)+1000)))
...
%  solution2=indice2+correction2;
  subplot(211)
  plot((solution1)/fs) % -solution2)/fs); % ranging solution
  xlabel('time (s)')
  ylabel('delay (s)')
  subplot(212)
  plot(diff(solution1)/fs) % -solution2)/fs); % ranging solution
  xlabel('time (s)')
  ylabel('diff(delay) (s)')
