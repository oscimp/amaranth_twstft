...
for dirnum=1:length(dirlist)
  nom=dirbit(mod(OP+remote+ranging,2)+1).name  % LTFB=odd OP=even
...
%       end
  ...
  if (ranging==1)
    eval(['save -mat rangingclaudio',nom,' corr* df indic* SNR* code puissan* xval*']);
  else
    if (remote!=1)
      eval(['save -mat localclaudio',nom,' corr* df indic* SNR* code puissan* xval*']);
    else
      eval(['save -mat remoteclaudio',nom,' corr* df indic* SNR* code puissa* xval*']);
    end
  end
