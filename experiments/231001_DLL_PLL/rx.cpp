...
typedef struct _channel_info // parameters
{
  ...
  double *dev_smp;   // received waveform on GPU memory, values in Volt
...
void char2double(int, int, char *, double *, double *);
void short2double(int, int, short *, double *, double *);
void PRN_sampling(int, int *, std::complex<double> *, int, double, int, double);
void PRN_mapping(int, int, int, std::complex<double> *, double *);
void downconv_acq(int, double, double, double *, std::complex<double> *, int);
void downconv_trk(int, int, double, double, double *, double *);
...
void MAI_out(int, double *, double *);
//
int main(int argc, char *argv[])
{
  ...
  bool is_chA, is_sic;
  double alpha, beta = 0.0, pwr_A, pwr_B;
  double *dev_obs, *dev_smp_A, *dev_smp_B, *dev_smp_MAI_free;
...
#if USRP == 310
  dev_samples=(short*)malloc(sizeof(short) * sps * 4); // NI USRP X310 I+jQ x 2 channels
#elif USRP == 210
  dev_samples=(char*)malloc(sizeof(char) * sps * 4); // NI USRP N210
#endif
  dev_obs=(double*)malloc(sizeof(double) * si.sps * 2);
  dev_smp_A=(double*)malloc(sizeof(double) * si.sps);
  dev_smp_B=(double*)malloc(sizeof(double) * si.sps);
...
#if USRP == 310
    short2double(si.sps, si.dec, dev_samples, dev_smp_A, dev_smp_B); // NI USRP X310
#elif USRP == 210
    char2double(si.sps, si.dec, dev_samples, dev_smp_A, dev_smp_B); // NI USRP N210
#endif
    pwr_A=cblas_ddot(si.sps / si.dec_a, dev_smp_A, si.dec_a, dev_smp_A, si.dec_a);
    // pwr_A=gsl_blas_ddot( dev_smp_A, si.dec_a, dev_smp_A, si.dec_a); TODO
//    printf("pwr_A %lf ",pwr_A);fflush(stdout);
    pwr_A /= (si.fs / (double)si.dec_a); // ch. A received power in <V^2>
//    printf("pwr_A/fs %lf \n",pwr_A);fflush(stdout);
    pwr_B=cblas_ddot(si.sps / si.dec_a, dev_smp_B, si.dec_a, dev_smp_B, si.dec_a);
//    printf("pwr_B %lf ",pwr_B);fflush(stdout);
...
//    printf("pwr_B/fs %lf\n",pwr_B);fflush(stdout);
    ...
    for (i = 0; i < si.nch; i++)
    {
      ...
      if (ci[i].is_sic == 0x1)
      {
        ...
        ci[i].px=cblas_ddot(si.sps / si.dec_a, dev_smp_MAI_free, si.dec_a, dev_smp_MAI_free, si.dec_a);
      ...
      if (ci[i].is_trk == 0x0)
      {
        ...
        while (1)
        {
          ...
          for (fcc = flow; fcc <= fhigh; fcc += fstep)
          {
            ...
            pk=cblas_dznrm2(1, ci[i].dev_obs_acq + (pk_idx ), 1); // cublasIzamax() returns [1:nfft], but we want [0:nfft-1] -> cblas n'a pas ce probleme
...
//        cublasDgemm(handle, CUBLAS_OP_T, CUBLAS_OP_N,      ci[i].nlag * 2 + 1, (ci[i].bps - 1) * 2, ci[i].nobs, &alpha, ci[i].dev_wav, ci[i].nobs, dev_obs, ci[i].nobs, &beta, ci[i].dev_res, ci[i].nlag * 2 + 1);
       ...
       cblas_dgemm(CblasColMajor, CblasTrans, CblasNoTrans, ci[i].nlag * 2 + 1, (ci[i].bps - 1) * 2, ci[i].nobs, alpha, ci[i].dev_wav, ci[i].nobs, dev_obs, ci[i].nobs, beta, ci[i].dev_res, ci[i].nlag * 2 + 1); // TODO VERIFIER CblasRowMajor -> CblasColMajor
...
void short2double(int nobs, int dec, short *smp, double *smpA, double *smpB)
{
  int i;
  for (i=0;i < nobs; i++)
  {
    smpA[i] = (double)smp[4 * i * dec] / 32768.0;     // JMF : real part of chA (+0)
    smpB[i] = (double)smp[4 * i * dec + 2] / 32768.0; // JMF : real part of chB (+2)
  }
}
...
void downconv_acq(int nobs, double ff, double phi, double *smp, std::complex<double> *res, int dec)
{
  ...
  for (i=0;i<nobs;i++)
  {
    ...
    // phi: initial phase
    res[i].real(1.4142135624 * smp[i * dec] * cos(-1.0 * 2.0 * PI * (ff * (double)i + phi)));
    res[i].imag(1.4142135624 * smp[i * dec] * sin(-1.0 * 2.0 * PI * (ff * (double)i + phi)));
  }
}

void downconv_trk(int nobs, int ld, double ff, double phi, double *smp, double *res)
{
  ...
  for (i=0;i < nobs;i++)
  {
    ...
    idx = (i / ld) * 2 * ld + i % ld;
    res[idx +  0] = 1.4142135624 * smp[i] * cos(-1.0 * 2.0 * PI * (ff * (double)i + phi));
    res[idx + ld] = 1.4142135624 * smp[i] * sin(-1.0 * 2.0 * PI * (ff * (double)i + phi));
  }
}
