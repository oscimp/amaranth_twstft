...
dop=dir([dirop,'*.*.*.txt*gz']);
for l=1:length(dop)
  eval(['op=load(''',dirop,dop(l).name,''');']);
  dltfb=dir([dirltfb,dop(l).name(1:8),'*ltfb.txt*gz']);
  if (isempty(dltfb)==0)
	...
	snroplo(m)=max(op(:,9));
	snroprem(m)=max(op(:,15));
	snrltfblo(m)=max(ltfb(:,9));
  	snrltfbrem(m)=max(ltfb(:,15));
      else
        if ((affiche==1)&&(m>length(dop)-5))
          figure
          subplot(311)
...
% if (affiche==1)
  figure
  k=find(temps>0);
  subplot(411);plot(temps(k)-59000,moy(k),'-');ylabel('<tw> (ns)')
  xlim([957 965])
  subplot(412);plot(temps(k)-59000,sta(k),'-');ylabel('std(tw) (ns)');xlabel('MJD-59000 (days)')
  xlim([957 965])
  k=find(vv>0);
  subplot(413);plot(temps(k)-59000,vv(k),'-');ylabel('FFT(tw)');xlabel('MJD-59000 (days)')
  xlim([957 965])
  subplot(414);plot(temps(k)-59000, snroplo(k));
  hold on;plot(temps(k)-59000, snroprem(k));
  plot(temps(k)-59000, snrltfblo(k));
  plot(temps(k)-59000, snrltfbrem(k));ylabel('SNR (dB)');xlabel('MJD-59000 (days)')
  legend('OP lo','OP rem','LTFB lo','LTFB rem')
  xlim([957 965])
% end
