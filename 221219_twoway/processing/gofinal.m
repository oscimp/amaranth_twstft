...
if (exist('args')==1)
  ...
  dirlistmatltfb=dir([localdirltfb,'/1*mat*']); % exclude remote*
end

for dirnummat=3:length(dirlistmatltfb)  % REPLACE WITH 1
%%% start LTFB: local
  filenameltfb=[localdirltfb,dirlistmatltfb(dirnummat).name];
  dateltfb=str2num(dirlistmatltfb(dirnummat).name(1:10));
  clear code
  eval(['load ',filenameltfb])
  solution1lo=indice1+correction1_a;          % ranging
  solution2lo=indice2+correction2_a;          % reference channel = loopback
  df1lo=df1;
  df2lo=df2;
...
% arbitrarily keep max(SNR)-10 to remove initial record when TX inactive
  k=find(10*log10(SNR2i+SNR2r)>max(10*log10(SNR2i+SNR2r))-10); 
  dateini=dateltfb-k(1)+1; % seconds since 1/1/1970 from loopback
  k=find(10*log10(SNR1i+SNR1r)>max(10*log10(SNR1i+SNR1r))-10); 
  dateiniltfb=dateltfb-k(1)+1; % seconds since 1/1/1970 from ranging
%%% start LTFB: remote
  ...
  foutname=strrep(dirlistmatltfb(dirnummat).name,'.mat.gz','lftb.txt');
  ...
  ltfbtw=(solution1remote-solution2lo)(k)/(2*Nint+1)/fs; % tw-loopback
  fclose(fout);
