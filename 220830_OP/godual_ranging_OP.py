...
def ranging(filename, prn_code):
    ...
    with open(filename, "rb") as fd:
        ...
        while(not must_stop):
            ...
            indice1_lst.append(indice1)
            indice2_lst.append(indice2)
            ...
            correction1.append((abs(xval1m1)-abs(xval1p1))/(abs(xval1m1)+abs(xval1p1)-2*abs(xval1))/2);
            correction2.append((abs(xval2m1)-abs(xval2p1))/(abs(xval2m1)+abs(xval2p1)-2*abs(xval2))/2);
            print(str(p)+": "+str(indice1)+"+"+str(correction1[-1])+" -- "+str(xval1))
            p += 1
